...
class Ithemes_Updater_Settings_Page {
	...
	private function save_settings() {
		...
		$this->messages[] = __( 'Settings saved', 'it-l10n-rcp-limited-quantity-available' );
	}
	...
	private function license_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username and password in order to license products.', 'it-l10n-rcp-limited-quantity-available' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username in order to license products.', 'it-l10n-rcp-limited-quantity-available' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership password in order to license products.', 'it-l10n-rcp-limited-quantity-available' );
		else if ( empty( $data['packages'] ) )
			$this->errors[] = __( 'You must select at least one product to license. Ensure that you select the products that you wish to license in the listing below.', 'it-l10n-rcp-limited-quantity-available' );
		...
		if ( empty( $response['packages'] ) ) {
			$this->errors[] = __( 'An unknown server error occurred. Please try to license your products again at another time.', 'it-l10n-rcp-limited-quantity-available' );
			return;
		}
		...
		foreach ( $response['packages'] as $package => $data ) {
			...
			if ( ! empty( $data['key'] ) )
				...
				$warn[$name] = __( 'Your product subscription has expired', 'it-l10n-rcp-limited-quantity-available' );
			else
		...
		if ( ! empty( $success ) )
			$this->messages[] = wp_sprintf( __( 'Successfully licensed %l.', 'it-l10n-rcp-limited-quantity-available' ), $success );

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason )
				$this->errors[] = sprintf( __( 'Unable to license %1$s. Reason: %2$s', 'it-l10n-rcp-limited-quantity-available' ), $name, $reason );
		}

		if ( ! empty( $warn ) ) {
			foreach ( $warn as $name => $reason )
				$this->soft_errors[] = sprintf( __( 'Unable to license %1$s. Reason: %2$s', 'it-l10n-rcp-limited-quantity-available' ), $name, $reason );
		}
	}
	...
	private function unlicense_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username and password in order to remove licenses.', 'it-l10n-rcp-limited-quantity-available' );
		else if ( empty( $data['username'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership username in order to remove licenses.', 'it-l10n-rcp-limited-quantity-available' );
		else if ( empty( $data['password'] ) )
			$this->errors[] = __( 'You must supply an iThemes membership password in order to remove licenses.', 'it-l10n-rcp-limited-quantity-available' );
		else if ( empty( $data['packages'] ) )
			$this->errors[] = __( 'You must select at least one license to remove. Ensure that you select the licenses that you wish to remove in the listing below.', 'it-l10n-rcp-limited-quantity-available' );
		...
		if ( empty( $response['packages'] ) ) {
			$this->errors[] = __( 'An unknown server error occurred. Please try to remove licenses from your products again at another time.', 'it-l10n-rcp-limited-quantity-available' );
			return;
		}
		...
		foreach ( $response['packages'] as $package => $data ) {
			...
			if ( isset( $data['status'] ) && ( 'inactive' == $data['status'] ) )
				...
				$fail[$name] = __( 'Unknown server error.', 'it-l10n-rcp-limited-quantity-available' );
		}


		if ( ! empty( $success ) )
			$this->messages[] = wp_sprintf( _n( 'Successfully removed license from %l.', 'Successfully removed licenses from %l.', count( $success ), 'it-l10n-rcp-limited-quantity-available' ), $success );

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason )
				$this->errors[] = sprintf( __( 'Unable to remove license from %1$s. Reason: %2$s', 'it-l10n-rcp-limited-quantity-available' ), $name, $reason );
		}
	}
	...
	public function list_packages( $action = 'list_packages', $post_data = array() ) {
		...
		if ( ! empty( $_REQUEST['updated_url'] ) ) {
			$this->messages[] = __( 'Successfully updated the Licensed URL.', 'it-l10n-rcp-limited-quantity-available' );
		}
...
?>
	<div class="wrap">
		<h2><?php _e( 'iThemes Licensing', 'it-l10n-rcp-limited-quantity-available' ); ?></h2>
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>">
		...
		<div id="ithemes-updater-settings">
			<h3 class="subtitle"><?php _e( 'Settings', 'it-l10n-rcp-limited-quantity-available' ); ?></h3>

			<table class="form-table">
				<tbody>
					<tr valign="top">
						<th scope="row">
							<?php _e( 'Licensed URL', 'it-l10n-rcp-limited-quantity-available' ); ?>
						</th>
						<td>
							<p>
								<code><?php echo $GLOBALS['ithemes-updater-settings']->get_licensed_site_url(); ?></code>
								<a href="<?php echo admin_url( 'options-general.php?page=ithemes-licensing&action=change_licensed_site_url' ); ?>" class="button button-primary"><?php _e( 'Change', 'it-l10n-rcp-limited-quantity-available' ); ?></a>
							</p>

							<?php if ( is_multisite() ) : ?>
								<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress network. If this is not set correctly, some features may not function as expected.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
							<?php else : ?>
								<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress site. If this is not set correctly, some features may not function as expected.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
							<?php endif; ?>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row">
							<label for="quick_releases"><?php _e( 'Quick Release Updates', 'it-l10n-rcp-limited-quantity-available' ); ?></label>
						</th>
						<td>
			...
			<p class="submit">
				<input id="save_settings" class="button button-primary" type="submit" value="<?php _e( 'Save Settings', 'it-l10n-rcp-limited-quantity-available' ); ?>" />
...
<?php
	...
	private function list_licensed_products( $products, $post_data, $action ) {
		...
		$headings = array(
			__( 'Product', 'it-l10n-rcp-limited-quantity-available' ),
			__( 'Member', 'it-l10n-rcp-limited-quantity-available' ),
			__( 'Expiration', 'it-l10n-rcp-limited-quantity-available' ),
			__( 'Remaining Licenses', 'it-l10n-rcp-limited-quantity-available' ),
		);
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-licensed">
			<h3 class="subtitle"><?php _e( 'Licensed Products', 'it-l10n-rcp-limited-quantity-available' ); ?></h3>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-1"><?php _e( 'Product', 'it-l10n-rcp-limited-quantity-available' ); ?></label>
						</th>
						<th scope="col"><?php _e( 'Member', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
						<th scope="col"><?php _e( 'Product Status', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
						<th scope="col"><?php _e( 'Expiration', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
						<th scope="col"><?php _e( 'Remaining Licenses', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php $count = 0; ?>
					<?php foreach ( $products as $name => $data ) : ?>
						<?php
							if ( -1 == $data['total'] )
								$remaining = __( 'unlimited', 'it-l10n-rcp-limited-quantity-available' );
							else
								$remaining = $data['total'] - $data['used'];

//							if ( 0 == $remaining )
//								$remaining .= ' <a class="button-secondary upgrade">' . __( 'Upgrade', 'it-l10n-rcp-limited-quantity-available' ) . '</a>';
				...
				<tfoot>
					<tr>
						<td colspan="6">
							...
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'Remove Licenses', 'it-l10n-rcp-limited-quantity-available' ); ?>" />
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-unlicensed">
			<h3 class="subtitle"><?php _e( 'Unlicensed Products', 'it-l10n-rcp-limited-quantity-available' ); ?></h3>

			<p><?php _e( 'The following products have not been licensed. Licensing a product gives you access to automatic updates from within WordPress.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
			<p><?php _e( 'To license products, select the products you wish to license, enter your iThemes membership username and password, and press the License Products button.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
			<p><?php printf( __( 'Need help? <a href="%s">Click here for a quick video tutorial</a>.', 'it-l10n-rcp-limited-quantity-available' ), 'https://ithemes.com/licensing/' ); ?></p>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col">
							<label for="cb-select-all-2"><?php _e( 'Product', 'it-l10n-rcp-limited-quantity-available' ); ?></label>
						</th>
					</tr>
				</thead>
				<tbody>
				...
				<tfoot>
					<tr>
						<td colspan="2">
							<input type="text" name="it-updater-username" placeholder="iThemes Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'License Products', 'it-l10n-rcp-limited-quantity-available' ); ?>" />
...
?>
	<div class="ithemes-updater-products" id="ithemes-updater-unrecognized">
		<h3 class="subtitle"><?php _e( 'Unrecognized Products', 'it-l10n-rcp-limited-quantity-available' ); ?></h3>

		<p><?php _e( 'The following products were not recognized by the licensing system. This can be due to a bug in the product code, a temporary server issue, or because the product is no longer supported.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
		<p><?php printf( __( 'Please check this page again at a later time to see if the problem resolves itself. If the product remains, please contact <a href="%s">iThemes support</a> and provide them with the details given below.', 'it-l10n-rcp-limited-quantity-available' ), 'https://ithemes.com/support/' ); ?></p>

		<table class="ithemes-updater-listing widefat">
			<thead>
				<tr>
					<th scope="col"><?php _e( 'Product', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
					<th scope="col"><?php _e( 'Type', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
					<th scope="col"><?php _e( 'Package', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
					<th scope="col"><?php _e( 'Version', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
					<th scope="col"><?php _e( 'Server Response', 'it-l10n-rcp-limited-quantity-available' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php $count = 0; ?>
				<?php foreach ( $products as $name => $data ) : ?>
					<?php
						if ( ( isset( $data['status'] ) && 'error' == $data['status'] ) && ( ! empty( $data['error']['message'] ) ) )
							...
							$response = __( 'Unknown Error', 'it-l10n-rcp-limited-quantity-available' );
...
?>
	<div class="wrap" id="ithemes-updater-site-url-confirmation">
		...
		<h2><?php _e( 'Licensing', 'it-l10n-rcp-limited-quantity-available' ); ?></h2>

		<p><?php _e( "Please confirm this site's licensed URL.", 'it-l10n-rcp-limited-quantity-available' ); ?></p>
		...
		<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>">
			...
			<div id="ithemes-updater-settings">
				<div id="ithemes-updater-table-wrapper">
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
									<label for="site_url"><?php _e( 'Licensed URL', 'it-l10n-rcp-limited-quantity-available' ); ?></label>

									<?php if ( is_multisite() ) : ?>
										<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress network.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
										<p class="description ithemes-updater-description-warning"><?php _e( 'If not set correctly, some features may not function as expected.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
									<?php else : ?>
										<p class="description"><?php _e( 'The Licensed URL should be the primary URL of this WordPress site.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
										<p class="description ithemes-updater-description-warning"><?php _e( 'If not set correctly, some features may not function as expected.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
									<?php endif; ?>
								</th>
								<td>
				...
				<p class="submit">
					<input id="save_licensed_site_url" class="button button-primary" type="submit" value="<?php _e( 'Save', 'it-l10n-rcp-limited-quantity-available' ); ?>" />
...
<?php
	...
	private function save_licensed_site_url( $data ) {
		...
		if ( empty( $data['site_url'] ) ) {
			$this->errors[] = __( 'The licensed URL cannot be blank.', 'it-l10n-rcp-limited-quantity-available' );
		} else if ( false === filter_var( $data['site_url'], FILTER_VALIDATE_URL ) ) {
			$this->errors[] = __( 'The licensed URL must be a valid URL.', 'it-l10n-rcp-limited-quantity-available' );
		}
		...
		$this->messages[] = __( 'Successfully set the Licensed URL.', 'it-l10n-rcp-limited-quantity-available' );
...
?>
	<div class="wrap" id="ithemes-updater-relicense">
		...
		<p><?php printf( __( 'The licenses on this site are for <code>%s</code>.', 'it-l10n-rcp-limited-quantity-available' ), $site_url_from_server ); ?></p>
		...
		<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo $this->self_url; ?>">
			...
			<div id="ithemes-updater-settings">
				<div id="ithemes-updater-table-wrapper">
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
									<label for="relicense_option">
										<?php _e( 'License Option', 'it-l10n-rcp-limited-quantity-available' ); ?>
									</label>
								</th>
								<td>
									<p>
										<label>
											...
											<?php _e( 'Create new licenses for this site.', 'it-l10n-rcp-limited-quantity-available' ); ?>
										</label>
									</p>
									<p class="description ithemes-updater-description-warning"><?php _e( 'Use this option if this site was cloned from another site and needs to have its own licenses.', 'it-l10n-rcp-limited-quantity-available' ); ?></p>
									<br />

									<p>
										<label>
											<input type="radio" name="relicense_option" value="update" <?php if ( 'update' === $data['relicense_option'] ) echo 'checked="checked"'; ?> />
											<?php printf( __( 'Change the existing licenses to be for <code>%s</code>.', 'it-l10n-rcp-limited-quantity-available' ), $data['site_url'] ); ?>
										</label>
									</p>
									<p class="description ithemes-updater-description-notice">
										<span class="dashicons dashicons-warning"></span>
										<?php printf( __( 'Note: If the <code>%s</code> site still exists and is different from this site, you will have to create new licenses on that site.', 'it-l10n-rcp-limited-quantity-available' ), $data['site_url'], $site_url_from_server ); ?>
									</p>
									<p class="description ithemes-updater-description-warning"><?php printf( __( 'Use this option if this site\'s primary URL has changed from <code>%1$s</code> to <code>%2$s</code>.', 'it-l10n-rcp-limited-quantity-available' ), $site_url_from_server, $data['site_url'] ); ?></p>
								</td>
							</tr>
							<tr valign="top">
								<th scope="row" colspan="2">
									<label for="it-updater-username"><?php _e( 'iThemes Username', 'it-l10n-rcp-limited-quantity-available' ); ?></label>
									<br />
							...
							<tr valign="top">
								<th scope="row" colspan="2">
									<label for="it-updater-password"><?php _e( 'iThemes Password', 'it-l10n-rcp-limited-quantity-available' ); ?></label>
									<br />
				...
				<p class="submit">
					<input id="relicense" class="button button-primary" type="submit" value="<?php _e( 'Save', 'it-l10n-rcp-limited-quantity-available' ); ?>" />
...
<?php
	...
	private function relicense( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply an iThemes membership username and password in order to change the licensed URL.', 'it-l10n-rcp-limited-quantity-available' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply an iThemes membership username in order to change the licensed URL.', 'it-l10n-rcp-limited-quantity-available' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply an iThemes membership password in order to change the licensed URL.', 'it-l10n-rcp-limited-quantity-available' );
		} else if ( empty( $data['site_url'] ) ) {
			$this->errors[] = __( 'The licensed URL cannot be blank.', 'it-l10n-rcp-limited-quantity-available' );
		} else if ( false === filter_var( $data['site_url'], FILTER_VALIDATE_URL ) ) {
			$this->errors[] = __( 'The licensed URL must be a valid URL.', 'it-l10n-rcp-limited-quantity-available' );
		} else if ( empty( $data['relicense_option'] ) || ! in_array( $data['relicense_option'], array( 'relicense', 'update' ) ) ) {
			$this->errors[] = __( 'You must pick one of the License Option options.', 'it-l10n-rcp-limited-quantity-available' );
		}
	...
	private function get_expiration_string( $expiration_timestamp ) {
		...
		if ( $time_left > ( 86400 * 30 ) )
			...
			if ( $time_left > 86400 )
				$expiration = sprintf( _n( '%d day', '%d days', intval( $time_left / 86400 ), 'it-l10n-rcp-limited-quantity-available' ), intval( $time_left / 86400 ) );
			else if ( $time_left > 3600 )
				$expiration = sprintf( _n( '%d hour', '%d hours', intval( $time_left / 3600 ), 'it-l10n-rcp-limited-quantity-available' ), intval( $time_left / 3600 ) );
			else if ( $time_left > 60 )
				$expiration = sprintf( _n( '%d minute', '%d minutes', intval( $time_left / 60 ), 'it-l10n-rcp-limited-quantity-available' ), intval( $time_left / 60 ) );
			else
				$expiration = sprintf( _n( '%d second', '%d seconds', $time_left, 'it-l10n-rcp-limited-quantity-available' ), intval( $time_left / 60 ) );

			if ( $expired )
				$expiration = sprintf( __( '%s ago', 'it-l10n-rcp-limited-quantity-available' ), $expiration );
		}
